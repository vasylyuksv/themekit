;(function ($, global) {
    var P = {
        ...
        _init: function () {
            ...
            if(global.LoyaltyConf) {
                self.$points = $('[data-lyt-points]');
                self.$transactionLoader = $('[data-transaction-loader]');
                self.$transactionTable = $('[data-transaction-table]');
                self.$transactionEmpty = $('[data-transaction-empty]');
                self.$pagination = $('#transactionPagination');

                self._getTransactions();
            }
        },

        _getTransactions: function() {
            var self = P;

            self._setLoading();

            $.ajax({
                url: global.LoyaltyConf.urlTransactions,
                type: 'post',
                dataType: 'json',
                data: JSON.stringify({
                    id: global.LoyaltyConf.customer.id,
                    email: global.LoyaltyConf.customer.email
                }),
                success: function (data) {
                    var balance = 0, transactions = [];

                    if(data.success) {
                        if(typeof data.data.summary.balance !== "undefined") {
        ...
        _setPagination: function() {
            ...
            if(self.transactions.length) {
                ...
                self._setTransactionTable([]);
            }

            if(self.transactions.length <= self.rowsPerPage) {
        ...
        _initPagination: function() {
            ...
            self.$pagination.paging(self.transactions.length, {
                format: '< ncnnn >',
                perpage: self.rowsPerPage, // show 10 elements per page
                lapping: 0, // don't overlap pages for the moment
                onSelect: function (page) {
                    var self = P, transactions, sliceStart, sliceEnd;
                    ...
                    transactions = self.transactions.slice(sliceStart, sliceEnd);
                    self._setTransactionTable(transactions);
                },
        ...
        _setTransactionTable: function(transactions) {
            var self = P, table;

            if(transactions.length) {
                table = $('<table></table>');
                $.each(transactions, function(key, data) {
                    var tr;

                    tr = $('<tr></tr>')
                        .append($('<td></td>', {text: data.type}))
                        .append($('<td></td>', {text: data.amount}))
                ...
                self.$transactionTable.find('tbody').html(table.html());
                self.$transactionTable.show();
            } else {
                self.$transactionEmpty.show();
            }

            self.$transactionLoader.hide();
        },

        _setLoading: function() {
            var self = P, img;

            img = $('<img />', {src: global.LoyaltyConf.urlLoader});
            self.$points.html(img);
        }
    };
