...
slate.Variants = (function() {
  ...
  Variants.prototype = _.assignIn({}, Variants.prototype, {
    ...
    _onSelectChange: function() {
     ...
     // END SWATCHES
      if(typeof isSticksProduct !== 'undefined' && isSticksProduct) {
			  $('.product-single__title').text(variant.title);
			  $('.product-single__description').html(variantAdditionalDesc[variant.id]);
        }
    },
    ...
    _updateImages: function(variant) {
      ...
      try {
        var imgSrc, imgCont, imgContId;
        if(typeof ProductVariantImg !== 'undefined') {
          imgSrc = ProductVariantImg[variant.id];
          
          if(imgSrc) {
            imgContId = '#FeaturedImageZoom-product-template-' + variantImage.id;
            imgCont = $(imgContId);
        ...
        console.log('Error custom image: ' + e);
      }
      // end custom product image
    },
...
theme.Cart = (function() {
  ...
  Cart.prototype = _.assignIn({}, Cart.prototype, {
    _setupCartTemplates: function() {
      ...
      this.$stickSuitableDevices = $(
        '[data-stick-suitable-cont]:first',
        this.$container
      ).clone();
    },
      ...
      $.post(params)
        .done(
          function(state) {
            ...
            window.GTM.changeQuantity(index,value)
    ...
    _countTotalBonuses: function(state) {
      var bonTotal = $('[data-cart-bonuses-subtotal]'), sum = 0;
      if(typeof CartConf !== 'undefined') {

        $.map(state.items, function(item) {
          if(typeof CartConf.variantBonuses[item.variant_id] !== 'undefined') {
            var itemBonusPrice = CartConf.variantBonuses[item.variant_id];
            sum += (item.quantity * itemBonusPrice);
          }
        });

        bonTotal.text(sum);

        if(typeof CartBonusesCheck !== 'undefined') {
          CartBonusesCheck.onPriceChange(sum);
        }
      }
    ...
    _createLineItemList: function(state) {
      var cartBonusSum, cartItems;

      cartBonusSum = this._countTotalBonuses(state);
      ...
      cartItems = $.map(
        ...
        function(item, index) {
          ...
          // set suitable stick devices
          if(this.$stickSuitableDevices && this.$stickSuitableDevices.length > 0 && typeof CartConf !== 'undefined') {
            var tags, iconData, isHyper, isDefault, $stickSuitableDevices, iconArr = [];

            $stickSuitableDevices = this.$stickSuitableDevices.clone();
            tags = CartConf.productTags[item.product_id];
            isHyper = tags.indexOf(CartConf.sticks.TAG_HYPER) > -1;
            isDefault = tags.indexOf(CartConf.sticks.TAG_DEFAULT) > -1;

            if(isHyper || isDefault) {
              iconData = CartConf.sticks.suitable_devices[(isDefault ? CartConf.sticks.TAG_DEFAULT : CartConf.sticks.TAG_HYPER)];

              $stickSuitableDevices.find('[data-devices]').removeClass(CartConf.sticks.suitable_devices_one_tag);
              if(iconData.length == 1) {
                $stickSuitableDevices.find('[data-devices]').addClass(CartConf.sticks.suitable_devices_one_tag);
              }

              $.each(iconData, function(index, value) {
                var _icon = '<div class="suitable-devices__product"><img src="' + value.img + '" alt="' + value.name + '"><span>' + value.name + '</span></div>';
                iconArr.push(_icon);
              });

              $stickSuitableDevices.find('[data-devices]').html(iconArr.join(''));

              $item.find('[data-stick-suitable-devices]').html($stickSuitableDevices);
            } else {
              $item.find('[data-stick-suitable-devices]').html('');
            }

          }
          // end suitable stick devices

          // bonuses
          (function() {
            if(typeof CartConf !== 'undefined') {
              if(typeof CartConf.variantBonuses[item.variant_id] !== 'undefined') {
                var itemBonusPrice = CartConf.variantBonuses[item.variant_id], bonus, totalBonus, notEnoughBonusError,
                  qntyPlusBtn;

                bonus = $item.find('[data-cart-item-bonuses]');
                totalBonus = $item.find('[data-cart-item-bonuses-total]');
                notEnoughBonusError = $item.find('[data-not-enough-bonus-error]');
                qntyPlusBtn = $item.find('[data-item-qnty-bnt="plus"]');

                bonus.show().find('[data-cart-item-bonuses-price]').text(itemBonusPrice);
                totalBonus.show().find('[data-cart-item-bonuses-price-total]').text(itemBonusPrice * item.quantity);
                if(!itemBonusPrice) {
                  bonus.hide();
                  totalBonus.hide();
                }

                if((CartConf.bonuses.balance - cartBonusSum - itemBonusPrice) < 0) {
                  notEnoughBonusError.show();
                  qntyPlusBtn.attr('disabled', true);
                } else {
                  notEnoughBonusError.hide();
                  qntyPlusBtn.attr('disabled', false);
                }
              }
            }
          })();
      ...
      if(typeof EsTracking !== 'undefined') {
        try {
          EsTracking.onCartChange();
      ...
      $.post(params)
        .done(
          function(state) {
            ...
            try {
              window.GTM.changeQuantity(index,0);
            } catch (e) {console.log(e);}
...
theme.Product = (function() {
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
      ...
      $(this.selectors.productForm, this.$container).on(
        ...
        function(evt) {
          ...
          if (!isInvalidQuantity && this.ajaxEnabled) {
            ...
            //window.GTM.addToCart($data.find('[name="id"]').val(), this.$quantityInput.val());
    ...
    _updateLiveRegion: function(evt) {
      ...
      window.GTMproduct.data.id = variant.sku;
      window.GTMproduct.data.price = variant.price/100;
      window.GTMproduct.data.variant = variant.title;
      window.GTMproduct.id = variant.id
      window.GTMproduct.detail();
...
theme.ProductRecommendations = (function() {
  function ProductRecommendations(container) {
    ...
    if (this.$container.data('productId').length > 1) {
    	var productId = this.$container.data('productId');
    } else {
    	var productId = 4336211853383; 
    }
...
// For SalesDoubler 
if ($.urlParam('aff_sub')) {
    createCookie('SD_clickid', $.urlParam('aff_sub'), 30);
}
if ($.urlParam('aff_id')) {
    createCookie('SD_aid', $.urlParam('aff_id'), 30);
}

// For ProLeads
if ($.urlParam('clickid')) {
    createCookie('PL_clickid', $.urlParam('clickid'), 30);
}

// For Sellaction 
if ($.urlParam('SAuid')) {
    createCookie('SA_clickid', $.urlParam('SAuid'), 30);
}
...
$('.new-stiks .list .item__btn').on('click', function () {
  if (!$(this).hasClass('active')) {
    $( '.new-stiks .list .item' ).removeClass('active');
  	$(this).addClass('active').siblings().removeClass('active');
    $(this).parent().siblings('.item__image').toggleClass('active');
  }
});
...
if ($(window).width() > 1026) {
	var myLi = $('#SiteNav > li:nth-child(2)');
  	myLi.find('.site-nav__dropdown').remove();
  	
    myLi.addClass('customDropdown');
    myLi.append($('.custom-dropdown'));

    //var mySvg = $('#SiteNav > li > a > svg')[0].clone();
    //$('.customDropdown > a').append(mySvg);
        ...
        $('.customDropdown').hover(
          function(e){
            ...
            setTimeoutConst = setTimeout(function () {
                $('.custom-dropdown').addClass('opened');
            }, 250); 
          },
            function() {
              var $this = $(this);
  ...
  $('#SiteNav .site-nav--has-dropdown').hover(function(){
  	$(this).toggleClass('opened');
  });
}
else {
	var myLi = $('.menu-scrollable__item--submenu');
    myLi.addClass('customDropdown');
    myLi.append($('.custom-dropdown'));
  ...
  var lastScrollTop = 0;
  var delta = 35;
  var navbarHeight = $('.menu-scrollable').outerHeight();
  ...
  function hasScrolled() {
      ...
      if (st > lastScrollTop && st > navbarHeight){
          $('.menu-scrollable').removeClass('nav-down').addClass('nav-up');
      } else {
          if(st + $(window).height() < $(document).height()) {
              $('.menu-scrollable').removeClass('nav-up').addClass('nav-down');
          }
      }
...
/***** FIX BODY paddingTOP from HEIGHT Header *****/
$(document).ready(function(){
  function headerCheck(){
  	var headerHeight = $('[data-section-id="header"]').height();
...
/***** FIX BODY TOP from HEIGHT Header *****/